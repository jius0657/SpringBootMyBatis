...
public class NoticeController {
     ...
     * 게시판 리스트 보여주기
     * <p>
     * GetMapping(value = "notice/noticeList") => GET방식을 통해 접속되는 URL이 notice/noticeList 경우 아래 함수를
     */
    ...
    public String noticeList(HttpSession session, ModelMap model)
        ...
        // 추후 구현할 것으로 가정하고, 공지사항 리스트 출력하는 함수에서 로그인 한 것처럼 Session 값을 생성함
        ...
        //로그 찍기(추후 찍은 로그를 통해 이 함수의 호출이 끝났는지 파악하기 용이하다.)
        log.info(this.getClass().getName(), "{}.noticeList End!");

        //함수 처리가 끝나고 보여줄 JSP 파일명
        // webopp/WEB-INF/views/notice/noticeList.jsp
     ...
     * 게시판 글 등록
     * <p>
     * 게시글 등록은 Ajax로 호풀되기 때문에결과는 JSON 구조로 전달해야만 함
    ...
    public String noticeInfo(HttpServletRequest request, ModelMap model) throws Exception {
        ...
        /*
         *###########################################################################
         * 반드시 값을 받았으면, 꼭 로그를 찍어서 값이 제대로 들어오는지 파악해야함 반드시 작성 할 것
         * ###########################################################################
         */
        log.info("nSeq : {}", nSeq);

        /*
         * 값 전은 반드시 DTO 객체를 이용해서 처리함 전달 받은 값을 DTO 객체에 넣는다.
         */
        NoticeDTO pDTO = new NoticeDTO();
        ...
        // 처리가 끝나고 보여줄 JSP 파일명
        // @Mapp/WER-INE/views/notice/noticeInfo.jsp
    ...
    public String noticeEditInfo(HttpServletRequest request, ModelMap model) throws Exception {
        ...
        */
        ...
        /*
         * 값은 반드시 DTO 객체를 이용해서 처리함 전달 받은 값을 DTO 객체에 넣는다.
         */
        NoticeDTO pDTO = new NoticeDTO();
        pDTO.setNoticeSeq(nSeq);
        ...
        // 함수 처리가 끝나고 보여줄 JSP 파일명
        // /WER-INE/views/notice/noticeEditInfo.jsp
        return "notice/noticeInfo";
    }
    ...
    public MsgDTO noticeUpdate(HttpSession session, HttpServletRequest request) {
        ...
        try {
            ...
            /*
             * ######################################################################
             *  반드시, 값을 받았으면, 꼭 로그를 찍어서 값이 들어오는지 파악해야함 반드시 작성할 것
             * #######################################################################
             */
